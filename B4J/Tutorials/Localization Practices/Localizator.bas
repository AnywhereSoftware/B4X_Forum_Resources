...
Sub LocalizeTabPane(tp As TabPane)
	...
	For index=tabs.size-1 To 0 Step -1
		Dim sm As JavaObject=jo.RunMethodJO("getSelectionModel",Null)
		sm.RunMethod("select",Array(index))
		Dim item As JavaObject=sm.RunMethod("getSelectedItem",Null)
		Dim page As TabPage=item
