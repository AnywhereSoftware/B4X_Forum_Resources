...
'Class module
Sub Class_Globals
	Type TreeItem2 (Children As List, Text As String)
...
Sub Handle(req As ServletRequest, resp As ServletResponse)
	...
	Try
		...
		code.Append("parser.Initialize(").Append(WebUtils.EscapeHtml("<text>")).Append(")").Append(" <br/>")
		code.Append("Dim root As ")
		Dim root As TreeItem2 = CreateTreeItem ("")
		If squareBracketFound Then
			code.Append("List = parser.NextArray").Append(" <br/>")
			BuildTree(parser.NextArray, root, code, "root", "", False, "")
		Else
			code.Append("Map = parser.NextObject").Append(" <br/>")
...
Sub BuildTree(element As Object, parent As TreeItem2, code As StringBuilder, _
	...
	If element Is Map Then
		...
		For Each k As String In m.Keys
			Dim ti As TreeItem2 = CreateTreeItem(k)
	...
	Else If element Is List Then
		...
		For Each e As Object In l
			Dim ti As TreeItem2 = CreateTreeItem(index)
			parent.Children.Add(ti)
	...
	End If
	If BuildList Then
		code.Append(indent.SubString(6)).Append("Next").Append(" <br/>")
	End If
End Sub
...
Sub WriteCodeHelper (ObjectType As String, code As StringBuilder, _
	...
	If GetFromMap.Length > 0 Then
		code.Append("Dim " & parentName & " As " & ObjectType & " = " & GetFromMap).Append(" <br/>")
	Else If BuildList Then
		code.Append("For Each " & parentName & " As " & ObjectType & " In " & parentName.SubString(3)).Append(" <br/>")
		indent = indent & "&nbsp;"
	End If
	Return indent
End Sub

Private Sub ConvertTreeToHtml(sb As StringBuilder, parent As TreeItem2, isRoot As Boolean)
	If isRoot = False Then sb.Append("<li>").Append(WebUtils.EscapeHtml(parent.Text))
	If parent.Children.Size > 0 Then
		sb.Append("<ul>")
		For Each ti As TreeItem2 In parent.Children
			sb.Append(CRLF)
...
End Sub
Private Sub CreateTreeItem (text As String) As TreeItem2
	Dim ti As TreeItem2
	ti.Initialize
