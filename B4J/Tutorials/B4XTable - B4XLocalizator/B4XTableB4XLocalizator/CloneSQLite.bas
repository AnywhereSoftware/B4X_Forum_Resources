...
Public Sub CopySQLiteDB(src As SQL, dest As SQL) As ResumableSub
...
Private Sub GetPlaceholders(Length As Int) As String
...
Private Sub GetObjectCreateSQL(DBconn As SQL, ObjectName As String) As String
'	Return DBconn.ExecQuerySingleResult($"SELECT sql FROM sqlite_schema WHERE name = '${ObjectName}'"$) 'Error si es en memoria
	Return DBconn.ExecQuerySingleResult($"SELECT sql FROM sqlite_master WHERE name = '${ObjectName}'"$)
End Sub

Private Sub GetTableList(DBconn As SQL, LowerCase As Boolean) As List
	Dim Cursor1 As ResultSet
...
Private Sub GetIndexList(DBconn As SQL, LowerCase As Boolean) As List
	Dim Cursor1 As ResultSet
