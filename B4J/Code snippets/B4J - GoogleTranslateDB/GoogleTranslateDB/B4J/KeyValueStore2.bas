...
Sub Class_Globals
	Private sql1 As SQL
	Private ser1 As B4XSerializator
...
Public Sub Put(Key As String, Value As Object)
	sql1.ExecNonQuery2("INSERT OR REPLACE INTO main VALUES(?, ?)", Array As Object(Key, ser1.ConvertObjectToBytes(Value)))
End Sub
...
Public Sub Get(Key As String) As Object
	...
	If rs.NextRow Then
		result = ser1.ConvertBytesToObject(rs.GetBlob2(0))
	End If
	rs.Close
	Return result
End Sub
...
Public Sub ContainsKey(Key As String) As Boolean
	Return sql1.ExecQuerySingleResult2("SELECT count(key) FROM main WHERE key = ?", Array As String(Key)) > 0
End Sub

#If KVSENCRYPT
Public Sub PutEncrypted (Key As String, Value As Object, Password As String)
	Dim cipher As B4XCipher
	Put(Key, cipher.Encrypt(ser1.ConvertObjectToBytes(Value), Password))
End Sub
...
Public Sub GetEncrypted (Key As String, Password As String) As Object
	...
	Return ser1.ConvertBytesToObject(cipher.Decrypt(b, Password))
End Sub
