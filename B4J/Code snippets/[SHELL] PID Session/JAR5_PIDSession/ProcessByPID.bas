...
Sub Class_Globals
	...
	Private safe As SafeByPID
	Dim Pathname, Filename As String
End Sub

'Initializes the object. You can add parameters to this method if needed.
Public Sub Initialize(iniPathname As String, iniFilename As String)
	ByPID.Initialize
	...
	Try
		If File.Exists(lstPathname, lstFilename) Then
			...
			If ByPID.IsInitialized And ByPID.Size > 0 Then
				...
				Next
				safe.Initialize
				Wait For (safe.Delete(lstPathname, lstFilename)) Complete (resultSafe As Boolean)
				If resultSafe Then Log("Safe delete!")
			Else
				Return False
			End If
		Else
