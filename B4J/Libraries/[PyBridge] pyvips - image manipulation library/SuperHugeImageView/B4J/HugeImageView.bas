...
Version=8.45
@EndOfDesignText@
'PyBridge version 1.00
#Event: Click
#Event: Progress (Visible As Boolean)
Sub Class_Globals
	...
	#Else If B4J
	Private FullImage As PyWrapper
	#end if
...
Private Sub GetZoomOutFile (Dir As String, FileName As String) As String
	Return File.Combine(File.DirTemp, File.GetName(File.Combine(Dir, FileName)) & "_zoomout.jpg")
End Sub

Public Sub SetBitmap(Dir As String, Filename As String, ReuseZoomOutImage As Boolean)
	Py = B4XPages.MainPage.Py
	pyvips = Py.ImportModule("pyvips")
	...
	#else if B4J
	Dim pb As B4XProgressDialog
	pb.Initialize(mBase.Parent)
	pb.ShowDialog("Loading image")
	FullImage = pyvips.GetField("Image").Run("new_from_file").Arg(File.Combine(Dir, Filename))
	Wait For (Py.FetchObjects(Array(FullImage.GetField("width"), FullImage.GetField("height")))) Complete (Values As List)
	ImageWidth = Values.Get(0)
	ImageHeight = Values.Get(1)
	Dim ZoomOutImage As B4XBitmap
	Dim ZoomOutFile As String = GetZoomOutFile(Dir, Filename)
	If ReuseZoomOutImage And File.Exists(ZoomOutFile, "") Then
		ZoomOutImage = xui.LoadBitmap(ZoomOutFile, "")
	Else
		Dim ScaledDownImage As PyWrapper = FullImage.Run("thumbnail_image").Arg(ZoomOutSize).Run("jpegsave_buffer")
		Wait For (ScaledDownImage.Fetch) Complete (Result As PyWrapper)
		ZoomOutImage = Py.ImageFromBytes(Result.Value)
		If ReuseZoomOutImage Then
			Dim out As OutputStream = File.OpenOutput(ZoomOutFile, "", False)
			ZoomOutImage.WriteToStream(out, 100, "JPEG")
			out.Close
		End If
	End If
	pb.Hide
	...
	XUIViewsUtils.SetBitmapAndFill(ZoomOutImageView, ZoomOutImage)
	Tiles.Initialize
	CreateTiles
	Reset
	ZoomChanged(0, 0, 0)
...
Private Sub SetImageViewLayout
	...
	Dim ShouldShowSmallTiles As Boolean = ZoomOutImageView.Width > ZoomOutSize * 1.2
	...
	For Each Tile As HugeTile In Tiles
		If ShouldShowSmallTiles And IsIntersect(Tile.SrcRect, VisibleRect) Then
			...
			If Tile.Loading = False Then
				If Tile.Image.IsInitialized = False Or Tile.Image.Width / Tile.ImageView.Width < 0.95 Then
...
Private Sub FillImages
	Dim Indices As List
	Indices.Initialize
	Dim WorkingTiles As List
	WorkingTiles.Initialize
	Dim rects As List
	rects.Initialize
	Dim RaiseProgressEvent As Boolean = xui.SubExists(mCallBack, mEventName & "_progress", 1)
	Do While True
		Indices.Clear
		WorkingTiles.Clear
		rects.Clear
		Do While TilesThatNeedToBeFilled.Size > 0
		...
		If Indices.Size > 0 Then
			If RaiseProgressEvent Then CallSubDelayed2(mCallBack, mEventName & "_progress", True)
			Wait For (GetCroppedImages(rects)) Complete (Images As List)
			For i = 0 To Indices.Size - 1
				Dim Tile As HugeTile = WorkingTiles.Get(i)
				If Tile.RSIndex = Indices.Get(i) Then
					Tile.Loading = False
					Tile.Image =  Py.ImageFromBytes(Images.Get(i))
					XUIViewsUtils.SetBitmapAndFill(Tile.ImageView, Tile.Image)
					Tile.ImageView.Visible = True
				End If
			Next
			If RaiseProgressEvent Then CallSubDelayed2(mCallBack, mEventName & "_progress", False)
		End If
		Sleep(IIf(TilesThatNeedToBeFilled.Size > 0, 10, 100))
	Loop
End Sub

Private Sub GetCroppedImages(rects As List) As ResumableSub
	Dim imgs As List
	imgs.Initialize
	For Each r As B4XRect In rects
		Dim img As PyWrapper = FullImage.Run("crop").Arg(r.Left).Arg(r.Top).Arg(r.Width).Arg(r.Height)
		If DisplayScale > 1 Then img = img.Run("resize").Arg(1 / DisplayScale)
		imgs.Add(img)
	Next
	Dim buffers As List
	buffers.Initialize
	For Each img As PyWrapper In imgs
		Dim buffer As PyWrapper = img.Run("jpegsave_buffer")
		buffers.Add(buffer)
	Next
	Wait For (Py.FetchObjects(buffers)) Complete (Result As List)
	Return Result
End Sub
