...
Sub Handle(req As ServletRequest, resp As ServletResponse)
	...
	Try
		...
		Else if method = "insert2" Then
			q = ExecuteInsert2(con, in , resp)
		Else
...
Private Sub ExecuteInsert2 (con As SQL, in As InputStream,  resp As ServletResponse) As String
	...
	If sqlStmt <> "" Then
		Try
			Dim jst As JavaObject
			jst.InitializeStatic("java.sql.Statement")
			Dim jps As JavaObject = jcon.RunMethod("prepareStatement", Array(sqlStmt, jst.GetField("RETURN_GENERATED_KEYS")))
			If cmd.Parameters <> Null Then
				For x = 0 To cmd.Parameters.Length - 1
					jps.RunMethod("setObject", Array(x+1, cmd.Parameters(x)))
				Next
			End If
			jps.RunMethod("executeUpdate", Null)
			rs = jps.RunMethod("getGeneratedKeys", Null)
		Catch
			If rs.IsInitialized Then rs.Close
			Log(LastException)
			errMsg = LastException.Message
	...
	End If
	If errMsg = "" Then
		Dim res As DBResult = ProcessGeneratedKeys(rs)
		rs.Close
		Dim data() As Byte = ser.ConvertObjectToBytes(res)
...
Private Sub ProcessGeneratedKeys(rs As ResultSet) As DBResult
	Dim jrs As JavaObject = rs
	Dim cols As Int = rs.ColumnCount
	Log("ColumnCount: " & cols)
	Dim res As DBResult
	...
	Do While rs.NextRow
		Dim row(cols) As Object
		For i = 0 To cols - 1
			'Note: Will return BIGINT type (-5)
'			row(i) = jrs.RunMethod("getLong", Array(i + 1))
			row(i) = jrs.RunMethodJo("getObject", Array(i + 1)).RunMethod("toString", Null)
			Log("Row " & i & " has value " & row(i))
		Next
		res.rows.Add(row)
	Loop
