...
Public Sub CreatePool2(driverClass As String, jdbcUrl As String, aUser As String, aPassword As String, poolSize As Int)
	...
	jo = jo.InitializeStatic("org.h2.jdbcx.JdbcConnectionPool")
	poolJO = jo.RunMethodJO("create", Array As Object(jdbcUrl, aUser, aPassword))
	If poolSize > 0 Then poolJO.RunMethod("setMaxConnections", Array As Object(poolSize))
End Sub
...
Public Sub SupportUrl(jdbcUrl As String) As Boolean
	If jdbcUrl.StartsWith("jdbc:h2:") Then Return True
	Log($"JdbcURL ${jdbcUrl} not supported by H2 connection pool"$)
	Return False
End Sub
