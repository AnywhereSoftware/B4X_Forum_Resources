...
Public Sub GetAllViewsRecursive As Object
	Return ActivityView.RootPanel.GetAllViewsRecursive
End Sub
...
'	'LogColor("Activity_Resize "&ActivityEvent,Colors.green)
	If Not(FirstTime) Then 
		ActivityView.RootPanel.RemoveAllViews
		CallSub2(parentModule,"Activity_Pause", False) 'signifies something like changing orientation
	End If
...
Public Sub getColor As Int
	Return ActivityView.RootPanel.Color
End Sub

Public Sub setColor(Color As Int)
	ActivityView.RootPanel.Color=Color
End Sub
...
Public Sub Activity_Pause(UserClosed As Boolean) As ResumableSub
'	'LogColor("Activity_Pause "&ActivityEvent,Colors.Magenta)
	If Not(parentModule=Null) Then 
