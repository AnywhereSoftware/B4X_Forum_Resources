...
Sub Globals
	...
	Private etTextBox As EditText					'Cell text box
	Private pnlNormal As Panel						'For Nomal buttoms
	...
	Private editLabel(60) As Label					' Sames number as Starter Headers list (could be a list)
	Private editBox(60) As EditText
	Private pnlbtnEdit As Panel
...
Sub Activity_Resume
	...
	headers.Initialize
	CreateDBTable
	LoadB4xTableData(Starter.DBName,Starter.TotColNumber)     'loads TableData from dbTable
...
Sub CreateDBTable
	...
	dbTable.NumberOfFrozenColumns=Starter.FrozenCols
	dbTable.ArrowsEnabledColor=0xFFFFFFFF
	For i = 0 To Starter.TotColNumber - 1
		dbTable.AddColumn(Starter.ColNames(i), dbTable.COLUMN_TYPE_TEXT)
	Next
End Sub
...
Public Sub LoadB4xTableData(DBname As Object,TotColNo As Int)
	...
	Do While rs.NextRow
		...
		For i = 0 To TotColNo-1
			row(i) = rs.GetString(Starter.ColNames(i))
		Next
		TableData.Add(row)
	Loop
	rs.Close
...
Sub SaveB4XTableData(DBName As Object)
	...
	'clears old data
	SQLUtil.DeleteDataSQLTable(Starter.SQL,DBName)     ' deletes the old dbtable
	...
	Do While rs.NextRow
		Dim RowList As List
		RowList.Initialize
		For i = 1 To Starter.TotColNumber           ' Column numbers a 1 to ............
	...
	For RowNo = 0 To ExistingData.Size-1
		...
		SQLUtil.AddRowValues (Starter.SQL,DBName, Starter.TotColNumber, RowList)
	Next
End Sub
...
Sub pnlEdit_Touch (Action As Int, X As Float, Y As Float)
	If Action = 0 Then
		MouseX=X
		If MouseX>pnlEdit.Width-200dip Then
			MouseX=pnlEdit.Width-220dip          ' Allows editbox to remain on screen
		End If
		MouseY=Y
	End If
End Sub
...
Sub RecordChangeRowID(RowID As Long, NewList As List)
	...
	For i = 0 To Starter.TotColNumber-2
		column = dbTable.GetColumn(Starter.ColNames(i))
		Query = Query & column.SQLID & " = ?, "
	Next
	column  = dbTable.GetColumn(Starter.ColNames(Starter.TotColNumber-1))
...
Public Sub DeleteRecord(RowIdNo As Int)
	...
	Msgbox2Async(Question, "Delete Row", "Yes", "", "No",Null, False)
...
Sub DuplicateRecord
	...
	Msgbox2Async(Question, "Duplicate Row", "Yes", "", "No",Null, False)
	Wait For Msgbox_Result (Result As Int)
	If Result = DialogResponse.POSITIVE Then
		RowChangeColourSelect
...
Sub btnEsc_Click
	SaveB4XTableData(Starter.DBName)  ' Saves b4xTable back to the DB
	dbTable.Clear
...
Sub EditPanelInit
	'Log("InitEditPanel")
	Private EditHeight = 40dip As Int
	Private EditWidth = 110dip As Int
	Private EditTop As Int
	Private EditLeft As Int
	pnlEdit.RemoveAllViews

	For Col = 0 To Starter.TotColNumber - 1
		' set up the column labels
		editLabel(Col).Initialize("")
		editLabel(Col).TextSize = 15
		editLabel(Col).Color=Colors.LightGray
		editLabel(Col).TextColor=Colors.Black
		editLabel(Col).Gravity=Bit.Or(Gravity.CENTER_VERTICAL, Gravity.CENTER_HORIZONTAL)
		editLabel(Col).Text = Starter.ColNames(Col)
		
		
		'Set up the edittext boxes
		editBox(Col).Initialize("edtEdit")
		editBox(Col).Color=Colors.White
		editBox(Col).TextColor=Colors.Black
		If Starter.ColDataTypes(Col)="TEXT" Then
			editBox(Col).InputType=editBox(Col).INPUT_TYPE_TEXT	                 'Adjusts the input keyboard depending on datatype
		Else
			editBox(Col).InputType=editBox(Col).INPUT_TYPE_NUMBERS
		End If
		editBox(Col).Gravity=Bit.Or(Gravity.BOTTOM, Gravity.LEFT)
		editBox(Col).TextSize=15
		...
		End Select
		pnlEdit.AddView(editLabel(Col), EditLeft, EditTop, EditWidth, EditHeight)
		pnlEdit.AddView(editBox(Col), EditLeft, EditTop+EditHeight+5dip, EditWidth, EditHeight)
	Next
End Sub
...
Sub btnAccept_Click
	AddList.Initialize
	For i = 0 To Starter.TotColNumber-1              'Optains
