...
' Last modified by: fredo 2018-08-29T06:02:37+00:00
...
Private Sub RaiseEvent(Success As Boolean)
	Log("GoogleOAuth2.RaiseEvent, Success=" & Success)
	'###fredo mod 'CallSubDelayed3(mTarget, mEventName & "_AccessTokenAvailable", Success, ti.AccessToken) ' error: ~e:java.lang.ClassCastException: java.lang.String cannot be cast to com.yourxyz.abc.googleoauth2$_tokeninformation
	CallSubDelayed3(mTarget, mEventName & "_AccessTokenAvailable", Success, ti)
End Sub
...
Private Sub GetTokenFromRefresh As ResumableSub
...
' Added by mw71
' ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
' 2017-08-26, --> https://www.b4x.com/android/forum/threads/problems-with-oauth.80174/page-2#post-526872
Sub TestConnect As ResumableSub
	...
	If h_tc.Success Then
		Log("#- x315, h_tc.GetString.Length=" &  h_tc.GetString.Length)
		strRet = h_tc.GetString
		ti.Valid=True
	Else
		'Log("#-  x271, OAuth, Getting access token from REFRESH token in TestConnect")
		Dim j_rt As HttpJob
		j_rt.Initialize("", Me)
		...
		If j_rt.Success Then
			Log("#-  x279, OAuth TestConnect, Refresh access token Success")
			strRet = j_rt.GetString
			Dim jp As JSONParser
			...
			SaveToken
		Else
			Log("#-  x289, OAuth TestConnect, Refresh Token ERROR")
		End If
		j_rt.Release
	End If
	'
	h_tc.Release
	CallSubDelayed2(mTarget, mEventName & "_TestFinish", strRet)
	Return Null
End Sub