...
'	Dim TOP_PADDING As Int = ITEM_MARGIN * 2
	
	Dim hasUnsavedChanges As Boolean = False
	
	Dim lastAutoScrollTime As Long
	Dim Const AUTO_SCROLL_INTERVAL As Int = 100    ' ms between scrolls
	Dim Const AUTO_SCROLL_STEP As Int = 16          ' dip per scroll
End Sub
...
'Called whenever a visible page disappears. B4XPage_Background - Called when the app Is moved To the background. 
Private Sub B4XPage_Background
	Log("Background")
'	SaveAll
End Sub

'Back button or gesture for UpdateItemText / UpdateItemIcon / UpdateItem
Private Sub B4XPage_CloseRequest As ResumableSub
	Log("CloseRequest")
	
'	Dim sf As Object = xui.Msgbox2Async("Save changes before leaving?", "Warning", "Yes", "", "No", Null)
...
Sub OverlayTouch_Touch(action As Int, x As Float, y As Float)
	Select Case action
		...
		Case TOUCH_MOVE
			...
			If isDragging Then
				Dim scrollOffset As Int = GetScrollOffset
				Dim minY As Float = TOP_PADDING * 1dip
				Dim maxY As Float = TOP_PADDING * 1dip + (panelList.Size - 1) * (ITEM_HEIGHT + ITEM_MARGIN) * 1dip
				Dim adjustedY As Float = Max(minY, Min(y + scrollOffset - dragOffsetY, maxY))
				
'				Dim scrollOffset As Int = GetScrollOffset
'				Dim adjustedY As Float = y + scrollOffset - dragOffsetY
				'
'				' Clamp: prevent panel from going above first or below last slot
'				Dim minY As Float = TOP_PADDING * 1dip
'				Dim maxY As Float = TOP_PADDING * 1dip + (panelList.Size - 1) * (ITEM_HEIGHT + ITEM_MARGIN) * 1dip
'				adjustedY = Max(minY, Min(adjustedY, maxY))
				...
				UpdateAutoScroll(y)		' Instead of AutoScrollIfNeeded(y)
			End If
...
'	RemoveItem(index)   ' Remove exactly the panel the user tapped with one click '<-- For test RemoveItem. Uninstall and reinstall the app for restart
'	Return              ' <-- Critical: list and overlays have changed; abort current flow ' Mandatory: list and overlays are now rebuilt
...
' Called from TOUCH_MOVE as before:
Private Sub UpdateAutoScroll(touchY As Float)
	Dim now As Long = DateTime.Now
	If now - lastAutoScrollTime < AUTO_SCROLL_INTERVAL Then Return  ' Too soon — skip
	...
	If touchY < EDGE_ZONE * 1dip Then
		ScrollPanelTo(Max(currentScroll - AUTO_SCROLL_STEP * 1dip, 0))
		lastAutoScrollTime = now
	Else If touchY > panelH - EDGE_ZONE * 1dip Then
		ScrollPanelTo(Min(currentScroll + AUTO_SCROLL_STEP * 1dip, maxScroll))
		lastAutoScrollTime = now
	End If
End Sub
...
' Use the center of the panel to calculate the index
