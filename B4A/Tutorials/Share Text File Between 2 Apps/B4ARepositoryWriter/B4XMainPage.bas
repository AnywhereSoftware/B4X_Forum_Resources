...
#End Region
'Ctrl + click to export as zip: ide://run?File=%B4X%\Zipper.jar&Args=B4ARepositoryWriter.zip
...
Private Sub Button1_Click
	...
	File.MakeDir(File.DirRootExternal, Directory)
	Log(File.Exists(File.DirRootExternal, Directory))
	Dim B4XRepository As String = File.Combine(File.DirRootExternal, Directory)
	Log(B4XRepository)
	
	File.WriteString(B4XRepository, "Test.txt", DateTime.Date(DateTime.Now) & " " & DateTime.Time(DateTime.Now))
