...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	TextFieldUser.TextField.SetColorAndBorder(xui.Color_White,2dip,xui.Color_Black,8dip)
...
Sub mqtt_Connected (Success As Boolean)
	If Success = False Then
		Log(LastException)
		xui.MsgboxAsync("Error","ChatMqtt")
	Else
		BTConnect.Text = "Connected"
		BTConnect.TextColor = Colors.Red
...
'				BTConnect.TextColor = Colors.Black
'				TextFieldUser.TextField.Enabled = True
'				mqtt.Unsubscribe(SubscribeTopic)
'				mqtt.Close
'			End If
		End If
	Else
...
'EXIT APP
'If (Back Key) exit app
Sub B4XPage_CloseRequest As ResumableSub
	Dim sf As Object = xui.Msgbox2Async("Exit ChatMqtt?", "ChatMqtt", "Yes", "", "No", Null)
