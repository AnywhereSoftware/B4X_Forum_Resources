...
import io.github.tapcard.emvnfccard.exception.TlvException;
...
public final class TlvUtil {
	...
	public static String prettyPrintAPDUResponse(final byte[] data, final int indentLength) {
		...
		while (stream.available() > 0) {
			...
			//BA.Log("" + prettyPrintHex(tagBytes));                 //********************************************************************************************************
			tapcardWrapper.ba.raiseEventFromDifferentThread(BA.applicationContext, null, 0, tapcardWrapper.eventName + "_tag_data", true, new Object[]{tag.getName(), prettyPrintHex(tagBytes), prettyPrintHex(lengthBytes), prettyPrintHex(valueBytes)});
