...
Private Sub EvalHelper (expr As String) As Double
	...
	Do While m.Find
		...
		If Operator.Length > 0 Then
			If OperatorLevel.ContainsKey(Operator) = False Then
				Error = True
				Return 0
			End If
...
Private Sub BuildTree As ParsedNode
	...
	If rt.IsInitialized = False Then
		rt = pn
	End If
...
Private Sub EvalNode (pn As ParsedNode) As Double
	...
	'this is needed for in case the expression has no numbers, operators or brackets
