...
Version=7.01
@EndOfDesignText@
' this Version depends on DBUtils 2.11 from Erel
' it enhances the basic/standard functions

'DBUtils
Sub Process_Globals
	Type DBFieldInfo(FieldName As String, DataType As String, _
	...
	' #### extended Globals
	Public SQLDB As SQL
	Public SQLValues As Map
	Public SQLWHERE As Map
	Public SQLValueList As List
End Sub
...
Public Sub GetFieldsInfo(DB As SQL, TableName As String) As List
	...
	Dim FieldsList As List = ExecuteMemoryTable(DB, "PRAGMA table_info ('" & TableName & "')", Null, 0)
	For Each values() As String In FieldsList
		Dim FieldInfo As DBFieldInfo
...
' ####################### END OF Standard functionality of DBUtils #####################

' ####################### BEGIN OF extended Funcionality from TechDoc G. Becker ########

#region --- image ---
' this functions are usefull if you store the images as bit values in the data colum.
' Column type Blob required

' Image 2 Blob
' store Jpg or PNG image into Datacolumn Type Blob
public Sub Image2Blob(Image As Bitmap, PNG As Boolean) As Byte()
	Dim OutputStream1 As OutputStream
	OutputStream1.InitializeToBytesArray(1000)
	If PNG Then
		Image.WriteToStream(OutputStream1, 100, "PNG")
	Else
		Image.WriteToStream(OutputStream1, 90, "JPEG")
	End If
...
' Blob 2 Image
' get Blob value from Datacolumn and convert it to bitmap
public Sub Blob2Image(Blob() As Byte) As Bitmap
	Dim InputStream1 As InputStream
	InputStream1.InitializeFromBytesArray(Blob, 0, Blob.Length)
	Dim bmp As Bitmap
...
' this functions are usefull if you store all date and time values as
' ticks in the table column (colum type long required)

' convert a date to ticks
' store date as ticks to Datacolumn type long
public Sub Date2Ticks(Dateformat As String, DateValue As String) As Long
	DateTime.DateFormat = Dateformat
	Dim res As Long = DateTime.DateParse(DateValue)
	Return res
End Sub

' convert Ticks 2 Date
' build a datestring from ticks
public Sub Ticks2Date(DateFormat As String, DateValue As Long) As String
	DateTime.DateFormat=DateFormat
	Dim res As String = DateTime.Date(DateValue)
	Return res
End Sub

' convert a time to ticks
' store time as ticks to Datacolumn type long
public Sub Time2Ticks(TimeFormat As String, TimeValue As String) As Long
	DateTime.TimeFormat = TimeFormat
	Dim res As Long = DateTime.TimeParse(TimeValue)
	Return res
End Sub

' convert Ticks 2 Time
' build timesring from ticks
public Sub Ticks2Time(TimeFormat As String, TimeValue As Long) As String
	DateTime.TimeFormat=TimeFormat
	Dim res As String = DateTime.Time(TimeValue)
	Return res
End Sub

' convert a datetime to ticks
' store datetime stamp as ticks to Datcolumn Type long
public Sub DateTime2Ticks(DateFormat As String, TimeFormat As String, _
	DateValue As String, TimeValue As String) As Long
	DateTime.TimeFormat = TimeFormat
	DateTime.DateFormat = DateFormat
	Dim res As Long = DateTime.dateTimeParse(DateValue,TimeValue)
	Return res
End Sub

' convert ticks to datetime
' build datetime string from ticks
public Sub Ticks2DateTime(DateFormat As String, TimeFormat As String, _
	DateTimeValue As Long) As String
	DateTime.TimeFormat = TimeFormat
	DateTime.DateFormat = DateFormat
	Dim res As String = DateTime.Date(DateTimeValue) & " " _
...
#region --- encrpytion ---
' This code is from Erel
' Notice for totally encrpted Database use SQLiteCypher

' Build AES entcrypted Data to store in Blob Datacolumn
Public Sub EncryptText(text As String, password As String) As Byte()
	Dim c As B4XCipher
	Return c.Encrypt(text.GetBytes("utf8"), password)
End Sub

' decrypt data from Blob Datacolumn
...
#region --- sum/max/min/average ---
' this functions are usefull to get sum, max, min or everage values

' calculate values
' Action= avg,max,min,sum
' Preset SQLWhere (can be clear or size 1 or size 2 for AND condition)
' return -1 for error
Public Sub calcValue(Action As String, TableName As String, Column As String) As Object
	If SQLDB.IsInitialized Then
		Dim strSQL As String	
		strSQL=Column & "]) FROM [" & TableName & "]"
		Select Action
			Case "avg"
				strSQL ="SELECT avg([" & strSQL
			Case "max"
				strSQL = "SELECT max([" & strSQL
			Case "min"
				strSQL = "SELECT min([" & strSQL
			Case "sum"
				strSQL = "SELECT sum([" & strSQL
		End Select
		Dim res As Object = SQLDB.ExecQuerysingleresult(strSQL)
		Return res
	Else
		Return -1
	End If
...
#region --- info ---
' this functions are usefull to get additional info

' get row id from record
' Preset SQLWhere
' return -1 for error
public Sub getRowID(TableName As String) As Long
	If SQLDB.IsInitialized Then
		Dim rs As ResultSet
		rs = SQLDB.ExecQuery2("SELECT rowid FROM [" & TableName & "] WHERE [" _
		& SQLWHERE.GetKeyAt(0) & "] = ?", Array As String(SQLWHERE.GetvalueAt(0)))

		Dim res As Long = -1
		Do While rs.NextRow
			res = rs.GetLong("rowid")
		Loop
		rs.close
		Return res
	Else
...
' Preset SQLWhere
public Sub isUnique(TableName As String) As String
	If SQLDB.IsInitialized Then
		Dim Unique As String = "False"
		' query return Null if not found
		If SQLDB.ExecQuerySingleResult2( _
			"SELECT count(*) FROM [" & TableName & "] WHERE [" _
			& SQLWHERE.GetKeyAt(0) & "] = ?", _
			Array As String(SQLWHERE.GetValueAt(0))) <> Null Then 
			Unique = "False"
		Else
			Unique = "True"
		End If
...
' this functionality is usefull to maintain the database

' copy and open db
' Test=true file in access foldeer will be deleted
' Create=true if no db found empty db is created
public Sub openDB(FileName As String,Test As Boolean,Create As Boolean) As Boolean
	' delete file for testing purpose
	If File.Exists(GetDBFolder,FileName) And Test = True Then
		Log(File.Delete(GetDBFolder,FileName))
	End If
	' copy file to accessable folder
	If CopyDBFromAssets(FileName) <>"" Then
		' initialize process globals (open DB)
		SQLDB.Initialize(GetDBFolder,FileName,Create)
		SQLValues.Initialize ' MAP to hold new/updated column nalues key=ColumName, Value=Object
		SQLWHERE.Initialize ' MAP to hold WHERE Info key=COlumName, Value=constrain
		SQLValueList.Initialize ' List to hold set of value Maps for bulk operation
		If SQLDB.IsInitialized And SQLValues.IsInitialized And SQLWHERE.IsInitialized _
				And SQLValueList.IsInitialized Then
			Return True
		Else
			Return False
		End If
...
' close opened DB
public Sub closeDB
	If SQLDB.IsInitialized Then SQLDB.close
End Sub

' clean database and free space of deleted records
' return old SIze / new Size in KB
public Sub cleanDB(FIleName As String) As String
	If SQLDB.IsInitialized Then
		Dim fsOLD, fsNew As Long
		Dim res As String
		' get old size
		fsOLD = File.Size(GetDBFolder,FIleName)
		' clean DB
		SQLDB.ExecNonQuery("VACUUM")
		' get new size
		fsNew = File.Size(GetDBFolder,FIleName)
		' return result
		res = fsOLD & "/" & fsNew
		Return res
	Else
