...
'Return SDKVersion - same as Phone.SdkVersion
Private Sub GetSDKVersion As Int
	Dim jo As JavaObject
	jo.InitializeStatic("android.os.Build.VERSION")
	Return jo.GetField("SDK_INT")
End Sub

'Return whether non-market apps can be installed on device ; 1 = yes, 0 = no
Private Sub GetNMAppInst As String
	Dim jo As JavaObject
	jo.InitializeStatic("android.provider.Settings.Secure")
	Dim context As JavaObject
	context.InitializeContext
	Dim resolver As JavaObject = context.RunMethod("getContentResolver", Null)
	Return jo.RunMethod("getString", Array(resolver, "install_non_market_apps"))
End Sub
