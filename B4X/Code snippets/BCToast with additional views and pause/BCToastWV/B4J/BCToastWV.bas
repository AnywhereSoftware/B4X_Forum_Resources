...
Public Sub Initialize (Parent As B4XView)
	...
	BB1.ParseData.Views.Initialize
#if B4A
	Dim p As Panel = pnl
	p.Elevation = 5dip
#End If
...
Public Sub Show(Message As String, AddV As B4XView)
	If VHold.IsInitialized Then VHold.RemoveViewFromParent
	VHold = AddV
	...
	Next
	Dim VH As Int = 0, vw As Int = 0, vhpad As Int = 0, vwpad As Int = 0
	If AddV.IsInitialized Then
		'Dim BX As B4XView = BB1.ParseData.Views.Get(0)
		VH = AddV.Height
		vw = AddV.Width
		vwpad = PaddingSides / 2
		vhpad = PaddingTopBottom / 2
	End If
...
'	Log(iv.Height & "  " & VH)
	...
	Dim NewAdVLeft, NewAdvWidth As Int, BB1Left As Int
	If w > (vw + vwpad) Then
		NewAdvWidth = (vw + (2 * PaddingSides))
		NewAdVLeft = (w - NewAdvWidth) / 2
		BB1Left = PaddingSides
	Else
		NewAdvWidth = vw + vwpad
		NewAdVLeft = vwpad / 2
		BB1Left = (NewAdvWidth - iv.Width) / 2
	End If
	Dim NewLeft As Int = Parent.Width / 2 - Max(w, (vw + vwpad)) / 2
	pnl.SetLayoutAnimated(0, NewLeft, Parent.Height * VerticalCenterPercentage / 100 - (h + (VH + vhpad)) / 2, Max(w, vw + vwpad), h + (VH + vhpad))
	pnl.BringToFront
	Dim dx As Int = PaddingSides - iv.Left
	Dim dy As Int = PaddingTopBottom - iv.Top
	pnl.AddView(iv, BB1Left, PaddingTopBottom, iv.Width, iv.Height)
'	Log(iv.Top & ", " & iv.Height)
	...
	Loop
	BB1.mBase.Height = 20dip
	If AddV.IsInitialized Then
		BB1.mBase.Parent.AddView(AddV, NewAdVLeft, h, NewAdvWidth, VH)  'Max(h - (vh + vpad), vh))
	End If
