...
Sub Class_Globals
	...
	Private xpnlChart, xpnlCursor, xpnlToolbox, xpnlZoomBarH, xpnlZoomBarV, xpnlValues As B4XView
	Private xcvsChart, xcvsCursor, xcvsToolbox, xcvsZoomBarH, xcvsZoomBarV, xcvsValues As B4XCanvas
	Private xpnlEdit As B4XView
	Private edtTaskID, edtTaskName, edtTaskResponsible, edtTaskBeginDate, lblTaskRow, lblTaskEndDate, edtTaskDuration, edtTaskCompletion, edtTaskLagLead As B4XView
	Private btnTaskEditUpdate, btnTaskEditDelete, btnTaskEditInsertAt As B4XView
	Private lblTaskPredecessorID, lblTaskLagLead As B4XView
	Private xcbxTaskToEdit, xcbxTaskDependency, xcbxTaskPredecessorID As B4XComboBox
	Private xcpkTaskColor As xColorPicker
	Private rectButton(3) As B4XRect
	Private pthButton(3) As B4XPath
...
'	Gantt.GridColorDark = CalcDarkColor(Graph.GridColor)
	...
	xpnlToolbox = xui.CreatePanel("xpnlToolbox")
	mBase.AddView(xpnlToolbox, 0, 0, 1dip, 45dip)
	xcvsToolbox.Initialize(xpnlToolbox)
...
Public Sub Base_Resize (Width As Double, Height As Double)
	...
	xpnlToolbox.Width = Width
	xpnlToolbox.Top = Height - xpnlToolbox.Height
'	xpnlToolbox.SetColorAndBorder(Gantt.ChartBackgroundColor, 1, xui.Color_DarkGray, 0)
'	xpnlToolbox.SetColorAndBorder(Gantt.ChartBackgroundColor, 0, xui.Color_DarkGray, 0)
	xcvsToolbox.Resize(xpnlToolbox.Width, xpnlToolbox.Height)
	xcvsToolbox.DrawLine(0, 1dip, xpnlToolbox.Width, 1dip, xui.Color_LightGray, 1dip)
	ToolboxUpdate
	
	xpnlChart.Width = Width
	xpnlChart.Height = Height - xpnlToolbox.Height
	xpnlCursor.Width = Width
	xpnlCursor.Height = Height - xpnlToolbox.Height - ZoomH(0).BarWidth
	xpnlEdit.Width = Width
	xpnlEdit.Height = Height - xpnlToolbox.Height
	If edtTaskID.IsInitialized = False Then
		xpnlEdit.LoadLayout("TaskEdit")
		xpnlEdit.SetColorAndBorder(xui.Color_LightGray, 0, xui.Color_LightGray, 0)
...
'	xpnlZoomBarV.Left = Width - ZoomV.BarWidth
	...
	Log("Base_Resize " & Gantt.XNbIntervals)
	If Gantt.XNbIntervals > 0 Then
		...
		xpnlZoomBarH.Top = Height - ZoomH(0).BarWidth - xpnlToolbox.Height
...
'		xcvsZoomBarV.Resize(Width, Height - ZoomV.BarWidth)
	...
	Select Gantt.DisplayType
		Case "GANTT"
			ToolboxButtonUpdate(0)
		Case "TABLE"
			ToolboxButtonUpdate(1)
	End Select
End Sub
...
Private Sub InitGantt
	...
	Gantt.YNbIntervals = Min(Tasks.Size, (0.95 * mBase.Height - Gantt.Top - ZoomH(ChartIndex).BarWidth - xpnlToolbox.Height) / Gantt.YInterval - 1)
...
Private Sub ToolboxUpdate
	Private rextText As B4XRect
	Private x As Int
	Private SpaceV = 8dip As Int
	Private OffsetH = 100dip As Int
	Private SpaceH = 20dip As Int

	ButtonTextSize = xpnlToolbox.Height / 2.7 / xui.Scale
	ButtonFont = xui.CreateDefaultFont(ButtonTextSize)
	rextText = xcvsCursor.MeasureText("Table", ButtonFont)
	ButtonWidth = rextText.Width + 40dip
	ButtonTextY = xpnlToolbox.Height / 2 - rextText.CenterY
	xcvsToolbox.DrawRect(xcvsToolbox.TargetRect, Gantt.ChartBackgroundColor, True, 1dip)
	For i = 0 To ButtonText.Length - 1
		x = OffsetH + i * (SpaceH + ButtonWidth)
		rectButton(i).Initialize(x, SpaceV, x + ButtonWidth, xpnlToolbox.Height - SpaceV)
		pthButton(i).InitializeRoundedRect(rectButton(i), 5dip)
		xcvsToolbox.DrawPath(pthButton(i), xui.Color_DarkGray, False, 1dip)
		xcvsToolbox.DrawText(ButtonText(i), x + ButtonWidth / 2, ButtonTextY, ButtonFont, xui.Color_DarkGray, "CENTER")
	Next
	xcvsToolbox.Invalidate
End Sub

Private Sub ToolboxButtonUpdate(ButtonIndex As Int)
	xcvsToolbox.DrawPath(pthButton(ButtonCurrentIndex), ButtonBackgroundColor, True, 1dip)
	xcvsToolbox.DrawPath(pthButton(ButtonCurrentIndex), ButtonBorderColor, False, 1dip)
	xcvsToolbox.DrawText(ButtonText(ButtonCurrentIndex), rectButton(ButtonCurrentIndex).CenterX, ButtonTextY, ButtonFont, ButtonTextColor, "CENTER")
	ButtonCurrentIndex = ButtonIndex
	xcvsToolbox.DrawPath(pthButton(ButtonCurrentIndex), ButtonBackgroundSetColor, True, 1dip)
	xcvsToolbox.DrawPath(pthButton(ButtonCurrentIndex), ButtonBorderSetColor, False, 1dip)
	xcvsToolbox.DrawText(ButtonText(ButtonCurrentIndex), rectButton(ButtonCurrentIndex).CenterX, ButtonTextY, ButtonFont, ButtonTextSetColor, "CENTER")
	xcvsToolbox.Invalidate
End Sub
...
Private Sub ZoomBarResizeH
	...
	xpnlZoomBarH.Top = mBase.Height - ZoomH(ChartIndex).BarWidth - xpnlToolbox.Height
...
Private Sub xpnlToolbox_Touch (Action As Int, X As Float, Y As Float)
	Private i As Int
	...
	Select Action
		Case xpnlToolbox.TOUCH_ACTION_DOWN
			For i = 0 To rectButton.Length - 1
				If x >= rectButton(i).Left And x <= rectButton(i).Right And y >= rectButton(i).Top And y <= rectButton(i).Bottom Then
					ToolboxButtonUpdate(i)
					Select i
						Case 0
							xpnlEdit.Visible = False
							Gantt.DisplayType = "GANTT"
							DrawChart
						Case 1
							xpnlEdit.Visible = False
							Gantt.DisplayType = "TABLE"
							DrawChart
						Case 2
							EditFillTasks
							EditUpdate
...
Private Sub EditUpdate
	If CurrentTaskIndex >= 0 Then
		edtTaskID.Text = TaskToEdit.ID
		edtTaskName.Text = TaskToEdit.Name
		...
		xcpkTaskColor.Color= TaskToEdit.Color
		Private i As Int
		i = lstDependenciesShort.IndexOf(TaskToEdit.Dependency)
		xcbxTaskDependency.SelectedIndex = i
'		xcbxTaskDependency_SelectedIndexChanged(0)
		i = TaskIDs.IndexOf(TaskToEdit.PredecessorID)
...
'		xcbxTaskPredecessorID_SelectedIndexChanged(i)
		...
		Else
			...
			edtTaskBeginDate.Enabled = False
		End If
		btnTaskEditInsertAt.Text = "Insert at row: " & CurrentTaskIndex
		btnTaskEditInsertAt.Visible = True
		btnTaskEditUpdate.Visible = True
		btnTaskEditDelete.Visible = True
		lblTaskRow.Text = CurrentTaskIndex
	Else
		edtTaskID.Text = ""
		edtTaskName.Text = ""
		edtTaskResponsible.Text = ""
		...
		edtTaskLagLead.Visible = True
		edtTaskBeginDate.Enabled = True
		btnTaskEditInsertAt.Text = "Insert at row "
		btnTaskEditInsertAt.Visible = False
		btnTaskEditUpdate.Visible = False
		btnTaskEditDelete.Visible = False
	End If
End Sub
