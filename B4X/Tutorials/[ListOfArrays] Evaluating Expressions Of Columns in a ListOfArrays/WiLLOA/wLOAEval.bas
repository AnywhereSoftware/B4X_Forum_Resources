...
Public Sub Eval(LOA_ As ListOfArrays, colName As String, Expression As String)
	...
	Expression = Expression.Replace(" ", "").Replace("-(", "-1*(").ToLowerCase
	...
	If Not(LOA.mIndicesMap.containsKey(res)) Then 
		LOA.AddColumn(colName, temps.Get(res))
	End If
	Return
End Sub

'Comment
Public Sub Eval2(LOA_ As ListOfArrays, Expression As String) As List
	LOA = LOA_
	nrows = LOA.GetColumn(0).size
	temps.clear
	Error = False
