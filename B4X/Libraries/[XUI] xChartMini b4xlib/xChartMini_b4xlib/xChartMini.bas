...
Sub Class_Globals
	Type GraphDataM (Title As String, SubTitle As String, XAxisName As String, YAxisName As String, Left As Int, Right As Int, Top As Int, Bottom As Int, Width As Int, Height As Int, Radius As Float, CenterX As Int, CenterY As Int, Rect As B4XRect, YInterval As Int, XInterval As Double, XOffset As Int, BarWidth As Int, ChartType As String, BarSubWidth As Int, IncludeBarMeanLine As Boolean, IncludeValues As Boolean, ChartBackgroundColor As Int, GridFrameColor As Int, GridColor As Int, XScaleTextOrientation As String, GradientColors As Boolean, GradientColorsAlpha As Int, BarValueOrientation As String, BarMarginMode As Int)
	...
	Private mMaxDigits As Int
	Private AlreadyWrongChartType = False As Boolean
...
Public Sub AddBar(Name As String, BarColor As Int)
	If WrongChartType("BAR") Then Return
...
Public Sub AddLine(Name As String, LineColor As Int)
	If WrongChartType("LINE") Then Return
...
'		Graph.BarSubWidth = Graph.BarWidth / PD.YArray.Length
		...
		If Graph.BarMarginMode = 1 Then
			...
			Graph.Width = Graph.XInterval * Points.Size + Margin
...
Private Sub GetYScaleWidth(Index As Int) As Int
	...
	If Scale(sY(0)).Automatic = True Then
		Width = MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MaxVal, mMaxDigits), Texts.ScaleFont)
		For i = 1 To Scale(sY(Index)).NbIntervals
			Width = Max(Width, MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MinVal + i * Scale(sY(Index)).IntervalAuto, mMaxDigits), Texts.ScaleFont))
		Next
	Else
		Width = MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MaxVal, mMaxDigits), Texts.ScaleFont)
		Width = Max(Width, MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MinVal, mMaxDigits), Texts.ScaleFont))
		Width = Max(Width, MeasureTextWidth(NumberFormat3(Scale(sY(Index)).Interval, mMaxDigits), Texts.ScaleFont))
		Width = Max(Width, MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MaxVal - Scale(sY(Index)).Interval, mMaxDigits), Texts.ScaleFont))
		Width = Max(Width, MeasureTextWidth(NumberFormat3(Scale(sY(Index)).MinVal + Scale(sY(Index)).Interval, mMaxDigits), Texts.ScaleFont))
	End If
	Return Width
End Sub

Private Sub WrongChartType(Routine As String) As Boolean
	If Routine <> Graph.ChartType Then
		If Routine = "BAR" And Graph.ChartType = "STACKED_BAR" Then Return False
		If AlreadyWrongChartType = False Then
			LogColor("Chart incompatibility. You are using a " & Routine & " routine in a " & Graph.ChartType & " chart.", xui.Color_Red)
			Private Text As String
			Text = "You are using a " & Routine & " routine in a " & Graph.ChartType & " chart." & CRLF
			Text = Text & "You should change the chart type or" & CRLF
			Text = Text & "use routines for the specific chart"
			xui.MsgboxAsync(Text, "E R R O R  Chart incompatibility")
		End If
		AlreadyWrongChartType = True
		Return True
	Else
		Return False
	End If
...
Public Sub DrawChart
	If AlreadyWrongChartType = True Then Return
	...
	For i = 0 To Scale(sY(0)).NbIntervals
		...
		If Scale(sY(0)).DrawYScale = True Then
			...
			If MinMaxMeanValues(0) = 0 And MinMaxMeanValues(1) = 0 And Scale(sY(0)).Automatic = True Then
				txt = ""
			Else
				txt = NumberFormat3(Scale(sY(0)).MinVal + i * Scale(sY(0)).Interval, mMaxDigits)
			End If
...
Public Sub setMaxDigits(MaxDigits As Int)
	mMaxDigits = Min(10, Max(6, MaxDigits))
'	If Points.Size > 0 Then
'		InitValues
'	End If
End Sub
