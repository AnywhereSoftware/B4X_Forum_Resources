...
#DesignerProperty: Key: MultipleSelection, DisplayName: Multiple selection, FieldType: Boolean, DefaultValue: False, Description: Whether to allow multiple items selection.
#End Region
...
Sub Class_Globals
	...
	Private mMultipleSelection As Boolean
	Private mlstSelectedIndices As List
	Private mlstSelectedValues As List
End Sub
...
Public Sub Initialize (vCallBack As Object, vEventName As String)
	...
	mlstSelectedIndices.Initialize
	mlstSelectedValues.Initialize
...
Public Sub setMultipleSelection(Multiple As Boolean)
	mMultipleSelection = Multiple
End Sub
Public Sub getMultipleSelection As Boolean
	Return mMultipleSelection
End Sub

Public Sub getSelectedIndices As List
	Return mlstSelectedIndices
End Sub

Public Sub getSelectedValues As List
	Return mlstSelectedValues
End Sub
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	DesignerLabel = Lbl
	mMultipleSelection = Props.Get("MultipleSelection")
...
'Sets the value stored at the specified index.
...
Public Sub GetAllValues As List
	Dim lstResult As List
	lstResult.Initialize
	For i = 0 To items.Size - 1
		lstResult.Add(GetValue(i))
	Next
	Return lstResult
End Sub

Public Sub UnselectAll
	For Each ItemIndex As Int In mlstSelectedIndices
		SetPressedColor(GetRawListItem(ItemIndex).Panel, False)
	Next
	mlstSelectedIndices.Clear
	mlstSelectedValues.Clear
End Sub

Public Sub AddSelectedIndices(Indices As List)
	For Each ItemIndex As Int In Indices
		If mlstSelectedIndices.IndexOf(ItemIndex) = -1 Then
			SetPressedColor(GetRawListItem(ItemIndex).Panel, True)
			mlstSelectedIndices.Add(ItemIndex)
			mlstSelectedValues.Add(GetRawListItem(ItemIndex).Value)
		End If
	Next
End Sub
...
Private Sub PanelClickHandler(SenderPanel As B4XView)
	Dim ItemIndex As Int = SenderPanel.Tag
	Dim Value As Object = GetRawListItem(ItemIndex).Value
	Dim IsPressedColor As Boolean = (Bit.And(0xff000000, PressedColor) <> 0)
	SetPressedColor(SenderPanel, IsPressedColor)
	
	If mMultipleSelection Then
		Dim IndexOf As Int = mlstSelectedIndices.IndexOf(ItemIndex)
		If IndexOf = -1 Then
			mlstSelectedIndices.Add(ItemIndex)
			mlstSelectedValues.Add(Value)
		Else
			mlstSelectedIndices.RemoveAt(IndexOf)
			mlstSelectedValues.RemoveAt(IndexOf)
			SetPressedColor(SenderPanel, False)
		End If
	Else
		Sleep(200)
		SetPressedColor(SenderPanel, False)
	...
	If xui.SubExists(CallBack, EventName & "_ItemClick", 2) Then
		CallSub3(CallBack, EventName & "_ItemClick", ItemIndex, Value)
	End If
End Sub

Private Sub SetPressedColor(SenderPanel As B4XView, Pressed As Boolean)
	...
	If Pressed And Not(IsTransparentPressedColor) Then
		SenderPanel.SetColorAnimated(50, clr, PressedColor)
	Else
		SenderPanel.SetColorAnimated(50, PressedColor, clr)
	End If
End Sub
