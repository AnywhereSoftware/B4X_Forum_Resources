...
#Event: Click
#Event: LongClick
#Event: CursorValueChanged(CursorIndex As Int, CursorValue As Double)
#RaisesSynchronousEvents: Click
#RaisesSynchronousEvents: LongClick
#RaisesSynchronousEvents: CursorValueChanged(CursorIndex As Int, CursorValue As Double)
'
...
#DesignerProperty: Key: CursorON, DisplayName: CursorON, FieldType: Boolean, DefaultValue: False, Description: displays cursor 0 or not.
#DesignerProperty: Key: CursorValue, DisplayName: CursorValue, FieldType: Int, DefaultValue: 5, Description: sets the value of cursor 0.
#DesignerProperty: Key: CursorColor, DisplayName: CursorColor, FieldType: Color, DefaultValue: 0xFF000000, Description: color of cursor 0.
#DesignerProperty: Key: Cursor1ON, DisplayName: Cursor1ON, FieldType: Boolean, DefaultValue: False, Description: displays cursor 1 or not.
#DesignerProperty: Key: Cursor1Value, DisplayName: Cursor1Value, FieldType: Int, DefaultValue: 5, Description: sets the value of cursor 1.
#DesignerProperty: Key: Cursor1Color, DisplayName: Cursor1Color, FieldType: Color, DefaultValue: 0xFFFF0000, Description: color of cursor 1.
#DesignerProperty: Key: Cursor2ON, DisplayName: Cursor2ON, FieldType: Boolean, DefaultValue: False, Description: displays cursor 2 or not.
#DesignerProperty: Key: Cursor2Value, DisplayName: Cursor2Value, FieldType: Int, DefaultValue: 5, Description: sets the value of cursor 2.
#DesignerProperty: Key: Cursor2Color, DisplayName: Cursor2Color, FieldType: Color, DefaultValue: 0xFF00FF00, Description: color of cursor 2.
#DesignerProperty: Key: CursorStep, DisplayName: CursorStep, FieldType: Int, DefaultValue: 0, Description: step of the cursor value when moving 0 = no step
...
Sub Class_Globals
	...
	Private cCursorOn(3) As Boolean
	Private CursorMove = False As Boolean
	Private cCursorPosition(3), cCursorColor(3) As Int
	Private cCursorValue(3) As Double
	Private cCursorStep As Double
...
Public Sub Initialize (Callback As Object, EventName As String)
	...
	cCursorColor(1) = 0xFFFF0000
	cCursorColor(2) = 0xFF00FF00
	cCursorStep = 0
End Sub
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	cCursorOn(1) = Props.GetDefault("Cursor1ON", False)
	cCursorValue(1) = Props.GetDefault("Cursor1Value", 5)
	cCursorColor(1) = xui.PaintOrColorToColor(Props.GetDefault("Cursor1Color", 0xFFFF0000))
	cCursorOn(2) = Props.GetDefault("Cursor2ON", False)
	cCursorValue(2) = Props.GetDefault("Cursor2Value", 5)
	cCursorColor(2) = xui.PaintOrColorToColor(Props.GetDefault("Cursor2Color", 0xFF00FF00))
	cCursorStep = Props.GetDefault("CursorStep", 0)
...
Public Sub getCursor1ON As Boolean
	Return cCursorOn(1)
End Sub

Public Sub setCursor1ON(Cursor1ON As Boolean)
	cCursorOn(1) = Cursor1ON
	DrawCursors
End Sub
...
'sets or gets the Cursor1Value property
Public Sub getCursor1Value As Int
	Return cCursorValue(1)
End Sub

Public Sub setCursor1Value(Cursor1Value As Int)
	cCursorValue(1) = Cursor1Value
	DrawCursors
End Sub

'sets or gets the Cursor2Value property
Public Sub getCursor2Value As Int
	Return cCursorValue(2)
End Sub

Public Sub setCursor2Value(Cursor2Value As Int)
	cCursorValue(2) = Cursor2Value
	DrawCursors
End Sub
...
Public Sub getCursor1Color As Int
	Return cCursorColor(1)
End Sub

Public Sub setCursor1Color(Cursor1Color As Int)
	cCursorColor(1) = Cursor1Color
	DrawCursors
End Sub

'sets or gets the Cursor2Color property
Public Sub getCursor2Color As Int
	Return cCursorColor(2)
End Sub

Public Sub setCursor2Color(Cursor2Color As Int)
	cCursorColor(2) = Cursor2Color
	DrawCursors
End Sub

'sets or gets the CursorStep property
'0 = no step
Public Sub getCursorStep As Double
	Return cCursorStep
End Sub

Public Sub setCursorStep(CursorStep As Double)
	cCursorStep = Abs(CursorStep)
End Sub
	...
	Else
		Select cGaugeType
			Case "Horizontal"
				...
				If cCursorStep > 0 Then
					cCursorValue(cCurrentCursorIndex) = Floor((cCursorValue(cCurrentCursorIndex) + cCursorStep / 2) / cCursorStep) * cCursorStep
				End If
		...
		Select Action
			...
			Case pnlAction.TOUCH_ACTION_MOVE
				DrawCursors
			Case pnlAction.TOUCH_ACTION_UP
				If xui.SubExists(mCallBack, mEventName & "_CursorValueChanged", 2) Then
					CallSub3(mCallBack, mEventName & "_CursorValueChanged", cCurrentCursorIndex, cCursorValue(cCurrentCursorIndex))
				End If
				CursorMove = False
		End Select
	End If
