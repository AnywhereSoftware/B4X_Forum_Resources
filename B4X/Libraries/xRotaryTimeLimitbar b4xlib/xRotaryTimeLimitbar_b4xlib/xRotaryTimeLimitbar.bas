...
#DesignerProperty: Key: NotUsedTimeColor, DisplayName: NotUsedTimeColor, FieldType: Color, DefaultValue: 0xFFA9A9A9, Description: Color of the ring outsides the possible work hours.
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	mNotUsedTimeColor = xui.PaintOrColorToColor(Props.GetDefault("NotUsedTimeColor", 0xFFA9A9A9))
...
Private Sub CalcTimes
	...
	Private IntervalMinutes As Double
	
	For i = 0 To 1
		SliderTime(i) = ConvertAngleToTime(SliderAngle(i))
		SliderTimeMinutes(i) = 360 + (SliderAngle(i) + 180) * 2
	Next
	IntervalMinutes = SliderTimeMinutes(1) - SliderTimeMinutes(0)
	Hours = Floor(IntervalMinutes / 60)
	Minutes = Floor(IntervalMinutes - Hours * 60)
...
'900 returns "15:00" or "03:00 PM" depending on the Mode24Hours proprty
...
Public Sub getMinTimeInterval As Int
...
'Color of the ring outsides the sliders when the time between the sliders is less than 12 hours
...
'You can define bitmaps for the sliders
