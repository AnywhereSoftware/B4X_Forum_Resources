...
Public Sub InitializeWithoutDesigner(parent As B4XView,backgroundcolor As Int,show_header As Boolean,show_bottom As Boolean,show_close_button As Boolean,width As Int, height As Int)
...
Private Sub Base_Resize (Width As Double, Height As Double)
	...
	mBase.SetColorAndBorder(back_color,0dip,xui.Color_Transparent,5dip)
  ...
  If showBottom = True Then
		...
		xpnl_bottom.Top = mBase.Height - 50dip
...
Private Sub create_top
	...
	xpnl_header.Color = xui.Color_Red
	xpnl_header.SetColorAndBorder(xui.Color_Red,0,xui.Color_Transparent,5dip)
...
#End If
	...
	#If B4A  
	xIconFont = xui.CreateFont(Typeface.CreateNew(Typeface.FONTAWESOME, Typeface.STYLE_NORMAL), 23) 
	#Else If B4i  
	xIconFont = Font.CreateFontAwesome (25) 
	#Else If B4J 
	xIconFont = fx.CreateFontAwesome(25)
	#End If 
...
Private Sub create_bottom
	...
	mBase.AddView(xpnl_bottom,0,mBase.Height - 50dip,mBase.Width,50dip)
	xpnl_bottom.Color = xui.Color_Red
	xpnl_bottom.SetColorAndBorder(xui.Color_Red,0,xui.Color_Transparent,5dip)
	...
	xlbl_action_1.Font = xui.CreateDefaultBoldFont(15)
	xlbl_action_2.Font = xui.CreateDefaultBoldFont(15)
	xlbl_action_3.Font = xui.CreateDefaultBoldFont(15)
...
Private Sub xiv_icon_MouseClicked (EventData As MouseEvent)
...
#ELSE 
Private Sub xpnl_header_Touch(Action As Int, X As Float, Y As Float) As Boolean
#END IF
	If dragable = 1 Then
...
#ELSE 
Private Sub xpnl_content_Touch(Action As Int, X As Float, Y As Float) As Boolean
#END IF
...
Private Sub xpnl_close_MouseClicked (EventData As MouseEvent)
...
Private Sub xlbl_action_1_MouseClicked (EventData As MouseEvent)
	
	result(xlbl_action_1.Tag)
	
End Sub

Private Sub xlbl_action_2_MouseClicked (EventData As MouseEvent)
	
	result(xlbl_action_2.Tag)
	
End Sub

Private Sub xlbl_action_3_MouseClicked (EventData As MouseEvent)
