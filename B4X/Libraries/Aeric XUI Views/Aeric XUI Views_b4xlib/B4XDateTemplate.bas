...
Sub Class_Globals
	...
	Public HighlightedColor As Int = 0xFFBDD7FF 'soft blue hover tint
	Public SelectedColor As Int = 0xFF5B8DEF  'soft blue selected
	Public DaysInMonthColor As Int = 0xFF2D3748 'dark slate - readable on light bg
	Public DaysInWeekColor As Int = 0xFF8898AA  'medium grey header
...
Private Sub HandleMouse (x As Double, y As Double, move As Boolean)
	...
	If move Then
		...
		If validDay Then
			...
			DrawBox(cvsBackground, HighlightedColor, False, boxX, boxY)
		End If
	Else
		cvsBackground.ClearRect(cvsBackground.TargetRect)
		If validDay Then
