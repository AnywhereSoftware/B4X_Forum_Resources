...
'#Macro: Title, Export State, ide://run?File=%B4X%\CodeBundle.jar&Args=%STATE1%&autosave=True
#Macro: Title, Update Manifest, ide://run?File=%ADDITIONAL%\..\B4X\manifest-writer.jar&Args=%PROJECT%\..&Args=%PROJECT%\..&Args=Author&Args=Aeric&Args=Version&Args=2.66&Args=B4J.DependsOn&Args=jBitmapCreator,+jXUI,+jDateUtils,+JavaObject,+xCustomListView,+B4XFormatter&Args=B4A.DependsOn&Args=BitmapCreator,+xui,+DateUtils,+xCustomListView,+IME,+JavaObject,+B4XFormatter&Args=B4i.DependsOn&Args=iBitmapCreator,+iXUI,+iDateUtils,+xCustomListView,+B4XFormatter
#Macro: Title, Create B4XLib, ide://run?File=%JAVABIN%\jar.exe&WorkingDirectory=..\..&Args=-cMf&Args=..\Aeric+XUI+Views.b4xlib&Args=manifest.txt&Args=*.bas&Args=B4J\*.bas&Args=Files\*.bal&Args=Files\*.bil&Args=Files\*.bjl&Args=Files\*.css
#Macro: Title, Release folder, ide://run?File=%WINDIR%\SysWOW64\explorer.exe&Args=%PROJECT%\..\..
#Macro: Title, Copy to AddLibs, ide://run?File=%COMSPEC%&Args=/c&Args=copy&Args=%PROJECT%\..\..\Aeric+XUI+Views.b4xlib&Args=%ADDITIONAL%\..\B4X
#End Region
...
' <link>Donate|https://paypal.me/aeric80/</link> | <link>Forum|https://www.b4x.com/android/forum/threads/b4x-aeric-xui-views.171008/</link> | <link>GitHub|https://github.com/pyhoon/Aeric-XUI-Views-B4X</link>
...
Private Sub B4XPage_Created (Root1 As B4XView)
	...
	xui.SetDataFolder("XUI Views Example") 'required in B4J to set the default writable folder.
	B4XPages.SetTitle(Me, "XUI Views Example")
...
Sub btnOptions_Click
	...
	If Result = xui.DialogResponse_Positive Then
		Dialog.Show($"You selected: ${options.SelectedItems}"$, "OK", "", "") 'ignore
	End If
End Sub
...
Private Sub B4XPage_CloseRequest As ResumableSub
	If xui.IsB4A And Dialog.Close(xui.DialogResponse_Cancel) Then Return False 'close dialog with back key
	Return True
End Sub
