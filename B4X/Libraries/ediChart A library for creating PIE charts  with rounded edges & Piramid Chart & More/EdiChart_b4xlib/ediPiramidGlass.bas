...
#DesignerProperty: Key: FirstColor, DisplayName: First Color, FieldType: Color, DefaultValue: #FFe4e4e4, Description:
...
private Sub InitColor
	...
	Ombra=bc.CreateBrushFromColor(OscuraColore(OscuraColore(clr1)))
...
Private Sub DrawValue(Value As Float)
	...
	#End If
		...
		Rhombus.LineTo(bc.mWidth,bc.mHeight-6dip)
		Rhombus.LineTo(0,bc.mHeight-6dip)
		'Rhombus.LineTo(0,bc.mHeight)
		'Rhombus.LineTo(100,100)
		bc.DrawPath2(Rhombus, Ombra, True, 0)
	
	
		'disegno il piano inclinato
		
		Dim Rhombus As BCPath
		'punto basso, punto a destra, punto a destra piu corto, punto centrale piu corto
		Rhombus.Initialize(0,bc.mHeight-6dip).LineTo(bc.mWidth,bc.mHeight-6dip)
		Rhombus.LineTo(bc.mWidth-50dip,bc.mHeight-30dip)
		Rhombus.LineTo(0+50dip,bc.mHeight-30dip)
		'Rhombus.LineTo(0,bc.mHeight)
		'Rhombus.LineTo(100,100)
		...
		larghezza=bc.mWidth/(listaFinale.Size+2.1)
		
		Dim areaMax As Float
		areaMax=(bc.mHeight-50dip)
		areaMax=areaMax*Value/100
		
		
		For i=listaFinale.Size-1 To 0 Step -1'listaFinale.Size-1
			...
			'posizionamento su asse 0
			'essendo la base a 20 dip non puo essere il valore inferiore a 20 dip per cui metto 21
			If ( bc.mHeight- valorelunghezza)>(bc.mHeight-13dip) Then
				valorelunghezza=14dip
			End If
			Dim Rhombus As BCPath
			Rhombus.Initialize(larghezza*(i+1),bc.mHeight - 13dip)
			Rhombus.LineTo(larghezza*(i+1.5)+0.5*larghezza/3, bc.mHeight-10dip)
			Rhombus.LineTo(larghezza*(i+1.5)+0.5*larghezza/3, bc.mHeight- valorelunghezza-  larghezza/3/2)
			'Rhombus.LineTo(larghezza*(i+1), bc.mHeight- valorelunghezza-  larghezza/3/2)
			...
			Dim Rhombus As BCPath
			Rhombus.Initialize(larghezza*(i+2)+0.5*larghezza/3,bc.mHeight-13dip)
			Rhombus.LineTo(larghezza*(i+1.5)+0.5*larghezza/3,bc.mHeight-10dip)
			'Rhombus.LineTo(larghezza*(i+1.5)+larghezza/3, bc.mHeight- valorelunghezza-  larghezza/10)
			Rhombus.LineTo(larghezza*(i+1.5)+0.5*larghezza/3, bc.mHeight- valorelunghezza-  larghezza/3/2)
			bc.DrawPath2(Rhombus, BrushOmbra.get(i), True, 0)
		
			
			'devo disegnare il rettangolo
			
			Dim Rhombus As BCPath
			'punto basso, punto a destra, punto a destra piu corto, punto centrale piu corto
			Rhombus.Initialize(0+larghezza*(i+1.2),bc.mHeight-25dip- valorelunghezza)
			Rhombus.LineTo(larghezza*(i+2),bc.mHeight-25dip- valorelunghezza)
			Rhombus.LineTo(larghezza*(i+2),bc.mHeight-45dip- valorelunghezza)
			Rhombus.LineTo(0+larghezza*(i+1.2),bc.mHeight-45dip- valorelunghezza)
			bc.DrawPath2(Rhombus, BrushOmbra.get(i), True, 0)
			'Dim g As Grafico
			
			Dim Rhombus As BCPath
			'punto basso, punto a destra, punto a destra piu corto, punto centrale piu corto
			Rhombus.Initialize(0+larghezza*(i+1.5),bc.mHeight-25dip- valorelunghezza)
			Rhombus.LineTo(larghezza*(i+1.65),bc.mHeight-25dip- valorelunghezza)
			Rhombus.LineTo(larghezza*(i+1.5)+0.5*larghezza/3,bc.mHeight-20dip- valorelunghezza)
			'Rhombus.LineTo(0+larghezza*(i+1.2),bc.mHeight-45dip- valorelunghezza)
			bc.DrawPath2(Rhombus, BrushOmbra.get(i), True, 0)
			'Dim g As Grafico
			...
			If VisualizzaLegenda = False Then
				...
				Else
					Dim Font1 As B4XFont = xui.CreateDefaultFont(14)
				End If
			
			Else
				Dim text As String = NumberFormat(g.Percentuale, 1, 1) & "%"
				Dim Font1 As B4XFont = xui.CreateDefaultFont(10)
			End If
			...
			If Percentuale=True Then
				canvas.DrawText(text, rect.CenterX+larghezza/6, bc.mHeight - valorelunghezza -30dip, Font1, xui.Color_White, "CENTER")
			End If
...
'	Next
	...
	'DisegnaGraficoTorta(listaFinale)
	'DrawValue(100)
	AvviaAnimazioneGrafico
End Sub

private Sub AvviaAnimazioneGrafico
	ValoreAnima=5
