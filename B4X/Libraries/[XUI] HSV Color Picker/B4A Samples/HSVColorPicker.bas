...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	mBase = Base
	BordersColor = xui.PaintOrColorToColor(Props.Get("BorderColor"))
...
Private Sub Base_Resize (Width As Double, Height As Double)
	...
	DrawHueBar
	DrawAlphaBar
	HueBarSelectedChanged (selectedH / 360 * HueBar.pnl.Height, False)
	AlphaBarSelectedChange (SelectedAlpha / 255 * AlphaBar.pnl.Width, False)
End Sub
...
Private Sub HueBarSelectedChanged (y As Float, RaiseEvent As Boolean)
...
Private Sub AlphaBarSelectedChange(x As Float, RaiseEvent As Boolean)
...
Private Sub Update (RaiseEvent As Boolean)
	DrawColors
	HandleSelectedColorChanged(DONT_CHANGE, DONT_CHANGE, RaiseEvent)
End Sub

Private Sub HandleSelectedColorChanged (x As Int, y As Int, RaiseEvent As Boolean)
	...
	If RaiseEvent Then
		CallSub2(mCallBack, mEventName & "_ColorChanged", getSelectedColor)
	End If
End Sub
...
#if B4J
Private Sub colors_MousePressed (EventData As MouseEvent)
	colors_MouseDragged(EventData)
End Sub

Private Sub colors_MouseDragged (EventData As MouseEvent)
	HandleSelectedColorChanged (EventData.X, EventData.Y, True)
End Sub

Private Sub HueBar_MousePressed (EventData As MouseEvent)
	HueBar_MouseDragged(EventData)
End Sub
Private Sub HueBar_MouseDragged (EventData As MouseEvent)
	HueBarSelectedChanged (EventData.Y, True)
End Sub

Private Sub Alpha_MousePressed (EventData As MouseEvent)
	Alpha_MouseDragged(EventData)
End Sub
Private Sub Alpha_MouseDragged (EventData As MouseEvent)
	AlphaBarSelectedChange(EventData.x, True)
End Sub
#else if B4A or B4i
Private Sub Colors_Touch (Action As Int, X As Float, Y As Float)
	HandleSelectedColorChanged(X, Y, True)
End Sub

Private Sub HueBar_Touch (Action As Int, X As Float, Y As Float)
	HueBarSelectedChanged(Y, True)
End Sub

Private Sub Alpha_Touch (Action As Int, X As Float, Y As Float)
	AlphaBarSelectedChange(x, True)
End Sub
#end if