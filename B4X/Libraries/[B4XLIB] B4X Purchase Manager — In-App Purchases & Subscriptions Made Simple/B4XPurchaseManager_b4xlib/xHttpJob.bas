...
Public Sub Initialize (Name As String, TargetModule As Object)
	...
	target = TargetModule
	#if B4A And RELEASE
	If xHttpUtils2Service.TempFolder = "" Then
		Dim jo As JavaObject
		jo.InitializeNewInstance(Application.PackageName & ".xhttputils2service", Null)
...
Public Sub PostBytes(Link As String, Data() As Byte)
	...
	End Try
	CallSubDelayed2(xHttpUtils2Service, "SubmitJob", Me)
End Sub
...
'Multipart upload functionality commented out - not needed for this library
'If you need multipart uploads, uncomment and restore the MultipartFileData type definition
'
...
Public Sub Release
#If B4A or B4J
	File.Delete(xHttpUtils2Service.TempFolder, taskId)
#End If
End Sub
...
Public Sub GetString2(Encoding As String) As String
	...
	tr.Initialize2(File.OpenInput(xHttpUtils2Service.TempFolder, taskId), Encoding)
...
Public Sub GetBitmap As Image
	Dim b As Image
	b = fx.LoadImage(xHttpUtils2Service.TempFolder, taskId)
	Return b
End Sub
...
Public Sub GetBitmap As Bitmap
	Dim b As Bitmap
	b = LoadBitmap(xHttpUtils2Service.TempFolder, taskId)
	Return b
End Sub
...
Public Sub GetBitmapSample(Width As Int, Height As Int) As Bitmap
	Return LoadBitmapSample(xHttpUtils2Service.TempFolder, taskId, Width, Height)
End Sub
...
Public Sub GetBitmapResize(Width As Int, Height As Int, KeepAspectRatio As Boolean) As Bitmap
	Return LoadBitmapResize(xHttpUtils2Service.TempFolder, taskId, Width, Height, KeepAspectRatio)
End Sub
#End If
...
Public Sub GetInputStream As InputStream
	Dim In As InputStream
	In = File.OpenInput(xHttpUtils2Service.TempFolder, taskId)
	Return In
End Sub
#Else If B4i
'Called by the service to get the request
