...
Sub Initialize(wificlient As WiFiSocket, IP() As Byte, Port As UInt, username As String, password As String)
	Log("Got in initialize")
	mqco.Initialize(username, password)
	ConnectMQTT(0, wificlient, IP, Port, username, password)
End Sub


Sub ConnectMQTT(iCount As Byte, wificlient As WiFiSocket, IP() As Byte, Port As UInt, username As String, password As String)
	...
	Else
		...
		If iCount + 1 < 3 Then
			Delay(2000)
			ConnectMQTT(iCount + 1, wificlient, IP, Port, username, password)
		End If
