...
namespace B4R
{
    ...
    //  client closed and  client = other
...
//    void HandlePacket(void* arg, AsyncClient* client, struct pbuf *pb)
    ...
    byte B4RAsyncClient::abort(uint32_t clientID)
    {
       AsyncClient* tmp = reinterpret_cast<AsyncClient*>(clientID);  
       return tmp->abort();
    };
    ...
    {
       AsyncServer* rcs = new (backend) AsyncServer((Byte*)IP->data, port);
