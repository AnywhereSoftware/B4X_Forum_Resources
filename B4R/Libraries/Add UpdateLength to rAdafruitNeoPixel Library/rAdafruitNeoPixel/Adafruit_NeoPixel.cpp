...
void Adafruit_NeoPixel::movePixelColorDirect(uint16_t t, uint16_t f)  {
  if(f < 0 || f >= numLEDs) return; // Out of bounds, return no color.
  if(t < 0 || t >= numLEDs) return; // Out of bounds, return no color.
  if(f == t)                return; // Both are the same nothing to do
  ...
  if(wOffset == rOffset) { // Is RGB-type device
	...
	tp[wOffset] = fp[wOffset];
  }

  tp[rOffset] = fp[rOffset];
  tp[gOffset] = fp[gOffset];
  tp[bOffset] = fp[bOffset];
}

uint32_t Adafruit_NeoPixel::getPixelColorDirect(uint16_t n) {
  if(n >= numLEDs) return 0; // Out of bounds, return no color.
...
void Adafruit_NeoPixel::setPixelColorDirect(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
void Adafruit_NeoPixel::setPixelColorDirect(
 uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
