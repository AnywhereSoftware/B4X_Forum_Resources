...
' Disclaimer: 	LEGO® is a trademark of the LEGO Group of companies which does not sponsor or endorse this project.
...
' client - BLE client
Public Sub SetOutputLevel(client As NimBLEClient)
...
' level - Power (-127..127)
Public Sub SetPowerLevel(client As NimBLEClient, port As Byte, level As Int)
...
Public Sub Stop(client As NimBLEClient, Port As Byte)
...
Public Sub PortValueRequest(client As NimBLEClient, port As Byte)
...
Public Sub NotifyDistance(client As NimBLEClient, port As Byte)
...
Public Sub NotifyColor(client As NimBLEClient, port As Byte)
